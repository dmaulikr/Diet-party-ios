...
- (void)setup {
    TWPhotoPickerController *twPhotoPickerController = [[TWPhotoPickerController alloc] init];
    twPhotoPickerController.cropBlock = ^(UIImage *image) {
        //do something
    ...
    [twPhotoPickerController.view setFrame:self.showView.bounds];
    [self addChildViewController:twPhotoPickerController];
    [self.showView addSubview:twPhotoPickerController.view];
    [twPhotoPickerController didMoveToParentViewController:self];
...
- (IBAction)gridEvent:(UIButton *)sender {
    if (sender.selected) {
        sender.selected = NO;
        [UIView animateWithDuration:0.2 delay:0.0 options:0 animations:^{
            self.ImgViewGrid.alpha = 1.0f;
...
- (IBAction)menuEvent:(UIButton *)sender {
    ...
    [self lineAnnimation:sender];
}

#pragma mark - LineAnnimation
- (void)lineAnnimation:(UIButton *)button {
    [UIView animateWithDuration:0.5 animations:^{
        CGRect newFrame = self.bottmLineView.frame;
        newFrame.size.width = button.frame.size.width;
        newFrame.origin.x = button.frame.origin.x;
        [self.bottmLineView setFrame:newFrame];
